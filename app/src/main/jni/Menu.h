...
Java_uk_lgl_modmenu_FloatingModMenuService_getInfo(JNIEnv *env, jobject thiz, jstring str) {
    ...
    if (info == std::string("TEXTHIDDEN"))
        ...
        return env->NewStringUTF(OBFUSCATE("<b>VIP MOD ONLY</b>"));
    else if (info == std::string("HEADING"))
        return env->NewStringUTF(OBFUSCATE("<b>Message me on Telegram to buy mods</b>"));
        ...
        return env->NewStringUTF(OBFUSCATE("[ _ ]"));

    // *** Themes *** //
    else if (info == std::string("HIDECLOSE_CL"))
        return env->NewStringUTF(OBFUSCATE("#FFEB3B"));
    else if (info == std::string("TEXTHIDECLOSE_CL"))
        return env->NewStringUTF(OBFUSCATE("#24292E"));
...
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_FloatingModMenuService_getTheme(JNIEnv *env, jobject thiz, jstring str) {
    const char *info = env->GetStringUTFChars(str, 0);
...
Java_uk_lgl_modmenu_FloatingModMenuService_isReady(JNIEnv *env, jclass clazz) {
    // Make one yourself ;)
}
JNIEXPORT jboolean JNICALL
Java_uk_lgl_modmenu_FloatingModMenuService_needEsp(JNIEnv *env, jclass clazz) {
    // Make one yourself ;)
}
JNIEXPORT void JNICALL
Java_uk_lgl_modmenu_FloatingModMenuService_DrawOn(JNIEnv *env, jclass clazz, jobject e_spview,
